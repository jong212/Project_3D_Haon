...
public class SkillB : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        weaponColliderScript.SkillB += OnTriggerEnterEventHandler;
    }
    ...
    private void OnTriggerEnterEventHandler(Collider otherCollider)
    {
        ...
        if (isSkillAttack)
        {
            ...
            {
                otherCollider.GetComponent<MonsterInfo>().TakeDamage(500);
            }
            if (otherCollider.tag == "Boss")
            {

                otherCollider.GetComponent<Boss>().TakeDamage(500);
            }
            // 스킬 공격 처리 코드 추가
        }
        else
